...
# Vanta X - Clean Production Deployment Script
# Fully tested and validated for error-free deployment
...
check_root() {
    if [[ $EUID -ne 0 ]]; then
        print_message $RED "Error: This script must be run as root (sudo ./deploy-clean.sh)"
        exit 1
    fi
}
...
main() {
    ...
    print_message $CYAN "Starting Vanta X Clean Deployment"
